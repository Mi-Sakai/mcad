...
## [0.11.1] - 2026-09-23 — 部品欄の列構成の見直し

部品欄プリセットに「部番」(照合番号)と「質量[kg]」を加え、列の並びと列幅を改めた。`製図規定.md` 2-5 も同時に
改訂した。保存データ(`.mcad` v7・DXF)は変わらない。

### 変更

- **部品欄プリセットの列構成**(`crates/mcad-app/src/bom.rs`): 様式A/C(170mm)は部番 10・品番 35・名称 35・個数 10・
  材質 30・質量[kg] 20・備考 30 の 7 列、様式B(120mm)は備考を除いた部番 10・品番 25・名称 25・個数 10・材質 30・
  質量[kg] 20 の 6 列。ユーザー定義様式は様式A/C の比で按分する。**部番が照合番号**(図中の風船の番号)、**品番は
  品目番号**(型番・購入品の品番・図番など)になった(従来は品番が照合番号)。質量は 1 個あたりの値で、列幅は
  「9999.000」が入る幅。列数が様式で変わるため `bom_preset_columns` の戻り値を静的スライスにした
- **既存の表への「部品表にする」**: 見出し行(最下段)が旧 5 列(品番・名称・個数・材質・備考)の並びと完全一致する
  表は、旧品番を部番へ、他を同じ意味の列へ移す(新しい品番・質量は空欄)。様式A/C・様式B の並びと完全一致する表は
  同じ名前の列どうしで移す。**値を黙って捨てる変換はしない**: 見出しが一致しない表に値があるとき、また新しい
  列構成に入らない列に値があるとき(例: 備考に値がある部品表を様式B へ切り替える)は変換せず、理由をステータス
  バーに出す(`apply_bom_preset` は `Result` を返す)。空の表(`K` で置いた直後など)は従来どおり変換できる。
  既存の図面を開いただけでは部品表は変わらない

### テスト

- 1167本 → 1170本(うち ignored 2本)
