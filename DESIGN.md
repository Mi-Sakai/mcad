...
| 37 | app: 注記の紙基準化 | `DIM_TEXT_PX`/`DIM_ARROW_PX` 廃止 → 紙 mm 定数(文字3.5・矢印3.0)× `k`、`dim_sizes(zoom)` を `dim_sizes(scale)` へ、Text 高さの紙 mm 解釈(判断4)、ツールプレビュー・ピック許容の追従、既存単体テストの定数更新 | implement-sonnet | 35a |
