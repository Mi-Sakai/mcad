...
1. **変換はShapeのメソッドとして mcad-geom に置く**: `translate` に加えて `rotate(center, angle)` /
   `mirror(axis)` を追加する。app 層での座標いじりを禁止し、`Command::ModifyEntity` / `AddEntity` 経由を維持する
2. **複製は「選択集合 → AddEntity の Batch」**: 元エンティティは変更しない。undo 1回で複製全体が消える(Batch 原子性)。
   ...
   - **2026-07-19 追記**: 移動もドラッグ方式から同じ2クリック配置へ変更(`M` キー、Placement 機構を
     ...
     これに伴いタスク19のミラー起動キーは `M` から変更する(候補: `Shift+M`。タスク19着手時に確定)。
3. **オフセットは対象を限定**: 線分・円・円弧は厳密解が単純。ポリラインの自己交差処理は複雑なため、
   M5 では「各セグメントの平行移動+隣接セグメントの交点接続」の単純方式とし、自己交差は仕様として許容(明記する)
4. **回転・ミラーの基準指定はツール化**: Select 中に R / M キーで基準点(回転)・軸2点(ミラー)を
   クリック指定する2段階ステートマシン。既存 Tool フレームワークの流儀に合わせる

#### タスク分割

| # | タスク | 内容 | 担当 | 依存 |
|---|---|---|---|---|
| 17 | geom変換メソッド | `Shape::rotate` / `Shape::mirror` を追加、proptest で往復不変性(rotate→逆rotate等)を固定 | implement-opus | — |
| 18 | 複製 | Ctrl+D で複製配置モード(基準点→配置先の2クリック、プレビュー・スナップ統合)。選択集合を AddEntity の Batch で複製し、新集合を選択状態にする | implement-opus | — |
| 19 | 回転・ミラーツール | Select 内の R / M キー起点の基準指定ステートマシン、プレビュー描画、スナップ統合(ミラーのキーは変更予定、設計判断2の2026-07-19追記参照) | implement-opus | 17 |
