...
impl Vec2 {
    ...
    /// # ここだけ絶対 `EPS` である理由
    ///
    /// [`Vec2`] は「変位」であって位置を持たないので、相対化の基準になる座標スケールを
    /// 自分自身の長さ以外に持たない（`len <= EPS * (1 + len)` は実質 `len <= EPS` と
    /// 同じで、相対化しても何も変わらない）。したがって本メソッドのガードは
    /// 「0 除算を避けるための最終防衛線」でしかなく、**幾何的な退化判定には使わない**。
    /// 座標スケールを知っている呼び出し側（線分・弧など）は、先に
    /// [`crate::point_tol`] / [`crate::rel_tol`] で退化を判定すること。
    #[inline]
    #[must_use]
